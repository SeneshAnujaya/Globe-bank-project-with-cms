...
function validate_page($page) {
  ...
  if (is_blank($page['subject_id'])) {
    $errors[] = "Subject cannot be a blank.";
  }
...
function insert_page($page) {
  ...
  $sql .= ")";
  $result = mysqli_query($db, $sql);
  // For INSERT statemens, $result is true/false
  if ($result) {
    return true;
  } else {
    // INSERT Failed
...
function delete_page($id) {
  ...
  // For DELETE statemens, $result is true/false
  if ($result) {
    return true;
  } else {
    // DELETE Failed
...
// Find all admins order by last name, first name
...
function find_admin_by_id($id) {
  ...
  mysqli_free_result($result);
  return $admin; //returns and assoc arrray
...
if(is_blank($admin['password'])) {
  ...
  $errors[] = "password must be between 12 or more characters."; 
